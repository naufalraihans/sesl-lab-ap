...
class Memory {
  ...
  resolve(o) {
    let u = o >>> 0;
    if (u >= this.buffer.byteLength) {
      const masked = o & 0x7FFFFFFF;
      if (masked < this.buffer.byteLength) {
        return masked;
      }
    }
    return u;
  ...
  read8(o) { return this.u8[this.resolve(o)]; }
  read32(o) { return this.u32[this.resolve(o) >> 2]; }
  write8(o, v) { this.u8[this.resolve(o)] = v; }
  write32(o, v) { this.u32[this.resolve(o) >> 2] = v; }
  write64(o, vlo, vhi = 0) {
    const r = this.resolve(o);
    this.write32(r, vlo);
    this.write32(r + 4, vhi);
  }

  readStr(o, len) {
    return readStr(this.u8, this.resolve(o), len);
  }
  ...
  write(o, buf) {
    ...
    if (buf instanceof ArrayBuffer) {
      return this.write(r, new Uint8Array(buf));
    } else if (typeof buf === 'string') {
      return this.write(r, buf.split('').map(x => x.charCodeAt(0)));
    } else {
      const dst = new Uint8Array(this.buffer, r, buf.length);
      dst.set(buf);
      return buf.length;
    }
  }
...
class MemFS {
  constructor(options) {
    ...
    this.stdoutLenAtInputRequest = 0;
    this.inputOffsets = [];
  ...
  host_write(fd, iovs, iovs_len, nwritten_out) {
    ...
    this.hostWrite(str);
    this.stdoutStr = (this.stdoutStr || "") + str;
    return ESUCCESS;
  }
  ...
  host_read(fd, iovs, iovs_len, nread) {
    ...
    // Detect if we are starting to read a new input line from stdinStr
    if (this.stdinStrPos < this.stdinStr.length) {
      const isStartOfLine = this.stdinStrPos === 0 || this.stdinStr[this.stdinStrPos - 1] === '\n';
      if (isStartOfLine) {
        if (!this.inputOffsets) this.inputOffsets = [];
        this.inputOffsets.push((this.stdoutStr || "").length);
      }
    }
    ...
    if (this.stdinStrPos >= this.stdinStr.length) {
      if (!this.waitingForInput) {
        ...
        this.stdoutLenAtInputRequest = (this.stdoutStr || "").length;
      }
    }
    let readStr = '';
    for (let i = 0; i < iovs_len; ++i) {
      ...
      const chars = this.stdinStr.substr(this.stdinStrPos, lenToWrite);
      this.hostMem_.write(buf, chars);
      readStr += chars;
      size += lenToWrite;
    ...
    // Echo to stdout has been disabled here to prevent mixing stdin into stdout too early.
    // In xterm, input is drawn dynamically and interleaved in the main thread.
  ...
  copy_out(clang_dst, memfs_src, size) {
    ...
    let clang_dst_u = clang_dst >>> 0;
    if (clang_dst_u >= this.hostMem_.buffer.byteLength) {
      const masked = clang_dst & 0x7FFFFFFF;
      if (masked < this.hostMem_.buffer.byteLength) {
        clang_dst_u = masked;
      }
    }

    let memfs_src_u = memfs_src >>> 0;
    if (memfs_src_u >= this.mem.buffer.byteLength) {
      const masked = memfs_src & 0x7FFFFFFF;
      if (masked < this.mem.buffer.byteLength) {
        memfs_src_u = masked;
      }
    }

    // console.log(`[shared.js copy_out] clang_dst=${clang_dst} (resolved: ${clang_dst_u}), memfs_src=${memfs_src} (resolved: ${memfs_src_u}), size=${size}`);
    const maxHostSize = this.hostMem_.buffer.byteLength - clang_dst_u;
    const maxMemSize = this.mem.buffer.byteLength - memfs_src_u;
    const safeSize = Math.max(0, Math.min(size, maxHostSize, maxMemSize));
    
    if (safeSize > 0) {
      const dst = new Uint8Array(this.hostMem_.buffer, clang_dst_u, safeSize);
      const src = new Uint8Array(this.mem.buffer, memfs_src_u, safeSize);
      dst.set(src);
    }
  }
  ...
  copy_in(memfs_dst, clang_src, size) {
    ...
    let memfs_dst_u = memfs_dst >>> 0;
    if (memfs_dst_u >= this.mem.buffer.byteLength) {
      const masked = memfs_dst & 0x7FFFFFFF;
      if (masked < this.mem.buffer.byteLength) {
        memfs_dst_u = masked;
      }
    }

    let clang_src_u = clang_src >>> 0;
    if (clang_src_u >= this.hostMem_.buffer.byteLength) {
      const masked = clang_src & 0x7FFFFFFF;
      if (masked < this.hostMem_.buffer.byteLength) {
        clang_src_u = masked;
      }
    }

    // console.log(`[shared.js copy_in] memfs_dst=${memfs_dst} (resolved: ${memfs_dst_u}), clang_src=${clang_src} (resolved: ${clang_src_u}), size=${size}`);
    const maxMemSize = this.mem.buffer.byteLength - memfs_dst_u;
    const maxHostSize = this.hostMem_.buffer.byteLength - clang_src_u;
    const safeSize = Math.max(0, Math.min(size, maxMemSize, maxHostSize));
    
    if (safeSize > 0) {
      const dst = new Uint8Array(this.mem.buffer, memfs_dst_u, safeSize);
      const src = new Uint8Array(this.hostMem_.buffer, clang_src_u, safeSize);
      dst.set(src);
    }
  }
}
...
class App {
  ...
  clock_time_get(clock_id, precision, time_out) {
    this.mem.check();
    const nowBig = BigInt(Date.now()) * 1000000n;
    const vlo = Number(nowBig & 0xffffffffn);
    const vhi = Number(nowBig >> 32n);
    this.mem.write64(time_out, vlo, vhi);
    return ESUCCESS;
  }
...
class API {
  ...
  async run(module, ...args) {
    // Commented out to prevent showing internal run commands (like > main.wasm) to the user
